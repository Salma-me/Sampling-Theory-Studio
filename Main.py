...
class MainApp(QtWidgets.QMainWindow, GUI.Ui_MainWindow):
    def __init__(self):
        ...
        self.horizontalSlider_fs.setRange(2, 11)
        ...
        self.horizontalSlider_SNR.setValue(30)
        self.horizontalSlider_SNR.setInvertedAppearance(True)
    ...
    def viewSelectedSinusoidal(self):
        ...
        if selectedSignal:
            self.horizontalSlider_freq.setValue(selectedSignal.frequency)
            self.horizontalSlider_amplitude.setValue(selectedSignal.amplitude)
            self.horizontalSlider_phase.setValue(selectedSignal.phase)
...
############### Tab 2 ####################
    ...
    def sampleSignal(self):
        ...
        if self.radioButton_actual.isChecked():
            ...
            self.samplingFrequency = sliderValue
        else:
            # Get the value of the slider
            sliderValue = self.horizontalSlider_fs.value()
            self.samplingFrequency = sliderValue * fMax

        # Sample the signal using the specified sampling frequency
        samplingT = 1 / self.samplingFrequency
        samplesTimeValues = np.arange(0, 1, step=samplingT)

        samplesAmpValues = np.interp(samplesTimeValues, self.time, signalAmplitudeValues)
        ...
        errorSignal = self.originalSignalAmplitudes - reconstructedSignal
    ...
    def updateSliderLimits(self):
        ...
        if self.radioButton_actual.isChecked():
            ...
            previousSliderValue = (self.horizontalSlider_fs.value() - 1) * self.getMaxFrequency() + 1
            self.horizontalSlider_fs.setValue(previousSliderValue)
            # self.horizontalSlider_fs.setValue(self.horizontalSlider_fs.minimum())
        elif self.radioButton_normalized.isChecked():
            self.horizontalSlider_fs.setRange(2, 11)
            self.horizontalSlider_fs.setValue(int(previousSliderValue / self.getMaxFrequency() + 1))
            # self.horizontalSlider_fs.setValue(self.horizontalSlider_fs.minimum())
